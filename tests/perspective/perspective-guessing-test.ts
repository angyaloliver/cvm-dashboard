...
const createBoundingBox = (
  ...
  return new BoundingBox(vec2.fromValues(sx, syFoot), syHead - syFoot);
};
...
describe("perspective-guessing", function () {
  it("errorHandCrafted", function () {
    ...
    storage.registerBoundingBox(
      new BoundingBox(vec2.fromValues(0, -1), 1.7 / 3)
    ); // height, not y
    storage.registerBoundingBox(
  ...
  it("error", function () {
    ...
    const storage = new BoundingBoxStorage();
    storage.registerBoundingBox(
      createBoundingBox(pers, vec3.fromValues(1, 1.7, 1))
    );
    storage.registerBoundingBox(
      createBoundingBox(pers, vec3.fromValues(1, 1.7, 2))
    );
    storage.registerBoundingBox(
  ...
  it("guessParams", function () {
    ...
    storage.registerBoundingBox(
      createBoundingBox(expectedResult, vec3.fromValues(1, 1.7, 1))
    );
    storage.registerBoundingBox(
      createBoundingBox(expectedResult, vec3.fromValues(-1, 1.7, 2))
    );
    storage.registerBoundingBox(
      createBoundingBox(expectedResult, vec3.fromValues(0.5, 1.7, 4))
    );
